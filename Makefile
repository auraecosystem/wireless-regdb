...
LSB_ID ?= $(shell if [ -f $(LSB_RELEASE) ]; then $(LSB_RELEASE) -i -s; else echo custom-distro; fi)
DISTRO_PRIVKEY ?= ~/.wireless-regdb-$(LSB_ID).key.priv.pem
DISTRO_PUBKEY ?= ~/.wireless-regdb-$(LSB_ID).key.priv.pem
REGDB_AUTHOR ?= $(shell if [ -f $(DISTRO_PRIVKEY) ]; then echo $(LSB_ID); elif [ -f $(WHOAMI) ]; then $(WHOAMI); else echo custom-user; fi)
REGDB_PRIVKEY ?= ~/.wireless-regdb-$(REGDB_AUTHOR).key.priv.pem
REGDB_PUBKEY ?= $(REGDB_AUTHOR).key.pub.pem
REGDB_PUBCERT ?= $(REGDB_AUTHOR).x509.pem
REGDB_UPSTREAM_PUBKEY ?= sforshee.key.pub.pem
REGDB_CHANGED = $(shell $(SHA1SUM) -c --status sha1sum.txt >/dev/null 2>&1; if [ $$? -ne 0 ]; then echo maintainer-clean $(REGDB_PUBKEY) $(REGDB_PUBCERT); fi)
...
regulatory.db.p7s: regulatory.db $(REGDB_PRIVKEY) $(REGDB_PUBCERT)
	...
	@openssl smime -sign -signer $(REGDB_PUBCERT) -inkey $(REGDB_PRIVKEY) -in $< -nosmimecap -binary -outform DER -out $@
sha1sum.txt: db.txt
	sha1sum $< > $@
...
$(REGDB_PUBCERT): $(REGDB_PRIVKEY)
	...
	sh gen-pubcert.sh $(REGDB_PRIVKEY) $(REGDB_PUBCERT) $(REGDB_AUTHOR)
...
install: regulatory.bin.5.gz regulatory.db.5.gz
	...
	if [ -f .custom ]; then install -m 644 -t $(DESTDIR)/$(CRDA_KEY_PATH)/ $(shell cat .custom); fi
